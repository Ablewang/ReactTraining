...
class PopupMove extends Component {
	...
	render() {
		return ( < Popup shade_close = {
				true
			}
			title = "请观看下面的视频"
			unique = {
				this.state.popup_unique
			}
			content = {
...
class ListTable extends Component {
	...
	componentWillMount() {
		...
		this.setPage(1);
		window.onresize = () => {
			let cur_max_col = this.getMaxColumn();
			if (cur_max_col != this.state.max_col) {
				this.setPage(this.state.cur_pg)
			}
		}
	}
	...
	render() {
		let maxColumn = this.state.max_col;
		let first = true;
